...
# Path: scripts/monotemporal.py
# Purpose: Train seasonal MaxEnt models using a 1x1 averaged predictor stack.
# Process Step: For each temporal rotation the predictor rasters of all
#               training years are averaged and used together with the
#               corresponding presence/background data. The remaining year is
#               evaluated as an independent test set.  The workflow mirrors the
#               multitemporal 3x2 script but omits spatial shifting and builds a
#               single averaged stack for training.
# Created by Adrian Meyer, Institute Geomatics, FHNW (adrian.meyer@fhnw.ch)
# Created: July 2025
# Version: v.0.1.0
# =============================================================================
"""Monotemporal training utility.

This script follows the same high level workflow as ``scripts/multitemporal``
but operates on a single 1×1 predictor stack.  For each temporal rotation the
predictor rasters of all training years (i.e. all years except the designated
test year) are averaged pixel-wise for every variable.  Presence and
background points from the remaining years are combined without any spatial
shifting.  The held out test year is processed separately using the original
year specific predictors.  Iterating across all years results in a
leave-one-year-out temporal cross-validation similar to the multitemporal
approach but without the 3×2 spatial matrix.

Running the script requires a configuration file mirroring the multitemporal
setup.  The same switches (e.g. optional hunting bag predictor) are supported.
"""
...
def _prepare_info_map(data: dict | None) -> dict[str, list[str]]:
    """Return a normalised copy of predictor drop metadata."""
    ...
    for key, values in data.items():
        ...
        if isinstance(values, (str, bytes)):
            ...
            seq = [v for v in values if v not in (None, "")]
        else:
            seq = [values]
        if not seq:
            continue
        seq = sorted({str(v) for v in seq})
        if seq:
            normalised[key] = seq
    return normalised


def _set_load_info(ds: xr.Dataset, info: dict[str, Iterable[str]]) -> xr.Dataset:
    """Attach predictor drop metadata to a dataset."""

    prepared = _prepare_info_map(info)
    if prepared:
        ds.attrs["_load_info"] = prepared
    elif "_load_info" in ds.attrs:
        del ds.attrs["_load_info"]
    return ds


def _normalise_var_name(name: str) -> str:
    """Collapse year tokens within ``name`` to a canonical form."""
...
def _build_coverage_summary(
    coverage: dict[str, dict[str, object]], years: Iterable[int]
) -> dict[str, object]:
    """Convert predictor coverage information into a serialisable summary."""
    ...
    for name, entry in sorted(coverage.items()):
        avail_years = sorted(int(y) for y in entry.get("available_years", set()))
        missing_src = entry.get("missing", {})
        missing: dict[str, str] = {}
        for year, reason in missing_src.items():
            try:
                year_int = int(year)
            except (TypeError, ValueError):
                continue
            missing[str(year_int)] = str(reason)
        record: dict[str, object] = {"available_years": avail_years, "missing": missing}
        if not avail_years and missing:
            record["excluded"] = True
        summary["variables"][name] = record
    return summary


def _log_predictor_coverage(summary: dict) -> None:
    """Emit warnings for predictors missing in some or all years."""

    years = [int(y) for y in summary.get("years", [])]
    total = len(years)
    variables = summary.get("variables", {})
    for name, data in sorted(variables.items()):
        if name == "boundary_mask":
            continue
        missing_entries: dict[str, str] = data.get("missing", {})
        available_years = set(int(y) for y in data.get("available_years", []))
        grouped: dict[str, list[int]] = {}
        for year_str, reason in missing_entries.items():
            try:
                year = int(year_str)
            except (TypeError, ValueError):
                continue
            if year in available_years:
                continue
            grouped.setdefault(reason, []).append(year)
        if data.get("excluded"):
            parts = [
                f"{reason}: {sorted(yrs)}"
                for reason, yrs in sorted(grouped.items())
                if yrs
            ]
            detail = "; ".join(parts) if parts else "no details"
            log.error(
                "Predictor %s unavailable in all %d training year(s); excluded (%s)",
                name,
                total,
                detail,
            )
            continue
        missing_years = sorted({year for yrs in grouped.values() for year in yrs})
        if not missing_years:
            continue
        parts = [
            f"{reason}: {sorted(yrs)}" for reason, yrs in sorted(grouped.items()) if yrs
        ]
        detail = "; ".join(parts) if parts else "no details"
        log.warning(
            "Predictor %s missing for %d/%d training year(s): %s",
            name,
            len(missing_years),
            total,
            detail,
...
def _load_year_stack(
    args: tuple[dict, str, YearOffset, bool]
) -> tuple[xr.Dataset, dict[str, list[str]]]:
    """Load predictor stack for a single year.

    Unlike the multitemporal variant no spatial shifting is applied and
    variables retain their original coordinates.  Year tokens within variable
    names are stripped so that subsequent averaging operates on common layer
    names.
    """

    cfg, season, yo, rebuild = args
    ds, _ = _load_stack(cfg, season, yo.year, rebuild, validate=rebuild)
    raw_info = copy.deepcopy(ds.attrs.get("_load_info", {}))
    ds_fused = fuse_yearly_layers(ds)

    reason_map = {
        "missing_expected": "missing_from_stack",
        "dropped_hunting": "disabled_by_config:hunting",
        "dropped_anomaly": "disabled_by_config:anomaly",
        "featureconfig_drop": "feature_config_drop",
    }
    info: dict[str, list[str]] = {}
    for key, names in raw_info.items():
        reason = reason_map.get(key, key)
        normalised = sorted({_normalise_var_name(name) for name in names})
        if normalised:
            info[reason] = normalised

    return ds_fused, info
...
@dataclass
class YearOffset:
    """Helper dataclass linking a training year.

    In the multitemporal implementation this class also carried spatial
    offsets.  For the monotemporal averaging approach no offsets are required
    but the structure is retained for minimal changes elsewhere in the code.
    """
...
def _experiment_dir(cfg: dict) -> Path:
    ...
    The default layout is ``<base>/<prefix>-<name>-<timestamp>`` where ``base``
    defaults to ``exps`` and ``prefix`` to ``monotemp``.  Both can be adjusted
    through ``experiment.base_dir`` and ``experiment.dir_prefix`` in the
    configuration.  A timestamp is only appended when the experiment name
    originates from the configuration (``append_timestamp`` defaults to ``True``
    in that case); auto generated names are already unique.  Repeated calls with
    configurations that share the same ``name`` (within the same Python
    process) therefore reuse the same output directory which keeps meta studies
    from rebuilding shared resources.
    """
    ...
    prefix = exp_cfg.get("dir_prefix", "monotemp")
...
def _build_matrix(years: list[int], test_year: int, shift: float) -> list[YearOffset]:
    """Return ordered list of training years.

    The signature mirrors the multitemporal variant but no spatial offsets are
    generated.  The returned list simply contains :class:`YearOffset` instances
    for all years except ``test_year`` in ascending order.
    """

    train_years = sorted(y for y in years if y != test_year)
    return [YearOffset(year=y) for y in train_years]
...
# -----------------------------------------------------------------------------
def _apply_feature_config(ds: xr.Dataset, cfg: dict) -> xr.Dataset:
    ...
    if Path(fc_path).exists():
        try:
            ...
            if drop:
                info.setdefault("featureconfig_drop", []).extend(drop)
                info = _prepare_info_map(info)
                ds = ds.drop_vars(drop)
                ds = _set_load_info(ds, info)
            else:
                ds = _set_load_info(ds, info)
...
def fuse_yearly_layers(ds: xr.Dataset) -> xr.Dataset:
    ...
    for name, da in ds.data_vars.items():
        ...
        if base != name:
            ...
            fused[base] = fused[base].combine_first(da)
        else:
            fused[name] = da
...
def _vif_filter(ds: xr.Dataset, cfg: dict) -> tuple[xr.Dataset, list[str]]:
    ...
    sample_frac = params.get("sample_fraction", 0.2)
    drop_cols = [c for c in ds.coords] + ["boundary_mask"]
    df = (
        ds.to_dataframe()
    ...
    if df.empty or len(all_vars) < 2:
        ...
        return ds, [v for v in ds.data_vars if v != "boundary_mask"]
    vif_vals = {
        var: variance_inflation_factor(df[all_vars].values, i)
...
def _merge_bias_maps(
    ...
    """Average bias rasters for all training years.

    If the configured pattern points to a single static bias map (e.g.
    ``Combined_All_Semesters.asc``) the raster is loaded once and returned
    without averaging.  Returns ``None`` if no bias rasters could be loaded.
    """

    bias_cfg = cfg.get("bias_grid", {})
    pattern = bias_cfg.get("pattern", "")
    if pattern and not any(tok in pattern for tok in ("{year}", "{season}")):
        da = _load_bias_raster(cfg, season, mapping[0].year if mapping else 0)
        if da is None:
            return None
        da = da.sortby("x")
    ...
    with ThreadPoolExecutor(max_workers=n_workers) as exe:
        for da in exe.map(lambda y: _load_bias_raster(cfg, season, y.year), mapping):
            if da is None:
                continue
            arrays.append(da)
    if not arrays:
        return None
    merged = xr.concat(arrays, dim="year").mean("year")
...
def _sample_background_bias(
    ...
    ``strength`` interpolates between uniform sampling (0) and full bias
    weighting (1).  If ``mask`` is provided, background points are restricted to
...
def _visualise_bias(
    ...
    """Save a visualisation of the bias map.

    The monotemporal workflow uses a single, combined bias raster for all
    seasons and years (``Combined_All_Semesters.asc``).  Consequently there is
    no need to handle per-year spatial offsets or annotations – the map is
    plotted once in its native coordinate reference system.
    """
    ...
    ax.set_title(f"{season} bias – test {test_year}")
    fig.tight_layout()
...
def _merge_stacks(
    ...
    """Average individual year stacks into a single training stack.

    Predictor layers for all training years are loaded, year prefixes are
    removed and the layers are averaged pixel-wise.  The resulting stack is
    written to ``out_dir/stack_name`` before any filtering is applied.
    ...
    years = [yo.year for yo in mapping]
    ...
    if zarr_path.exists():
        ...
        if meta_path.exists():
            try:
                ...
                _log_predictor_coverage(summary)
            except Exception as exc:  # pragma: no cover - diagnostics only
                log.warning(
                    "Failed to read predictor coverage information from %s: %s",
                    meta_path,
                    exc,
                )
    else:
        log.debug("Averaging %d yearly stacks using %d worker(s)", len(mapping), n_workers)
        load_args = [(cfg, season, yo, rebuild_years) for yo in mapping]
        if n_workers == 1:
            results = [_load_year_stack(args) for args in load_args]
        else:
            with ProcessPoolExecutor(max_workers=n_workers) as exe:
                results = list(exe.map(_load_year_stack, load_args))
        ...
        def ensure_entry(name: str) -> dict[str, object]:
            ...
            if entry is None:
                entry = {"available_years": set(), "missing": {}}
                coverage[name] = entry
            return entry

        var_templates: dict[str, xr.DataArray] = {}
        for (ds, info), year in zip(results, years):
            entry_vars = set()
            for name, da in ds.data_vars.items():
                entry = ensure_entry(name)
                entry.setdefault("available_years", set()).add(year)
                entry_vars.add(name)
                if name not in var_templates:
                    var_templates[name] = da
            for reason, names in info.items():
                for name in names:
                    entry = ensure_entry(name)
                    missing_map = entry.setdefault("missing", {})
                    missing_map[year] = reason
            # ensure metadata captures variables missing without explicit reason
            for name in var_templates:
                if name not in entry_vars:
                    entry = ensure_entry(name)
                    missing_map = entry.setdefault("missing", {})
                    missing_map.setdefault(year, "not_reported")

        union_vars = sorted(var_templates)
        filled: list[xr.Dataset] = []
        for (ds, _), year in zip(results, years):
            present = set(ds.data_vars)
            additions: dict[str, xr.DataArray] = {}
            for name in union_vars:
                if name not in present:
                    template = var_templates[name]
                    additions[name] = xr.full_like(template, np.nan, dtype="float64")
                    entry = ensure_entry(name)
                    missing_map = entry.setdefault("missing", {})
                    missing_map.setdefault(year, "not_reported")
            if additions:
                ds = xr.merge([ds, xr.Dataset(additions)], compat="override")
            ds = ds[union_vars]
            filled.append(ds)

        for entry in coverage.values():
            avail_years = entry.setdefault("available_years", set())
            missing_map = entry.setdefault("missing", {})
            for year in years:
                if year not in avail_years and year not in missing_map:
                    missing_map[year] = "not_reported"

        merged = xr.concat(filled, dim="year").mean("year", skipna=True)
        merged = merged.sortby("x").sortby("y", ascending=False)
        merged = _apply_feature_config(merged, cfg)
        summary = _build_coverage_summary(coverage, years)
        _log_predictor_coverage(summary)
    ...
    if fixed_vars is not None:
        ...
        vars_used = [v for v in merged.data_vars if v != "boundary_mask"]
    elif apply_vif:
...
def _visualise_points(
    ...
    """Plot presence or background points for the 1x1 configuration."""
...
def _predict_raster(
    ...
    preds[boundary.ravel()] = model.predict(grid[boundary.ravel()])
    ...
    # Compensate for the historic half-pixel offset of the preprocessing grid
    # so that generated cloglog rasters align with the predictor layers.
...
def process_season(cfg: dict, season: str, rebuild: bool) -> None:
    """Run monotemporal modelling for a single season."""
    ...
    for test_year in tqdm(test_years, desc=f"{season} CV", unit="year"):
        ...
        if bias_da is not None:
            ...
            _visualise_bias(bias_da, bounds, season, test_year, vis_dir)
        ...
        log.info("Preparing presence points for %d years", len(mapping))
        pres_list = [pres_raw[yo.year].copy() for yo in mapping]
        for gdf in pres_list:
            gdf["x"] = gdf.geometry.x
            gdf["y"] = gdf.geometry.y
        ...
        if grid_km:
            ...
            log.info(
                "Grid thinning %.0f km retained %d points",
                grid_km,
                len(pres_train),
            )
        log.info("Prepared %d presence points", len(pres_train))
        ...
        n_bg = bg_per_year * len(mapping)
        log.info("Sampling background points (%d per year)", bg_per_year)
        ...
        if (
            ...
            log.info("Using bias raster for background sampling")
            bg_train = _sample_background_bias(bias_da, n_bg, bias_strength, mask)
        else:
            log.info("Sampling background points uniformly")
            bg_train = _sample_background_polygon(fr_poly, n_bg)
        bg_train = deduplicate_cells(bg_train)
        bg_train = drop_background_overlaps(
            pres_train, bg_train, context="training background"
        )
        log.info("Training presences: %d", len(pres_train))
        log.info("Training background points: %d", len(bg_train))
        ...
        bias_test = _merge_bias_maps(
            cfg_run, season, [YearOffset(test_year)]
        )
        if (
            cfg_run.get("background", {}).get("use_bias_grid", False)
            ...
            bg_test = _sample_background_bias(bias_test, bg_per_year, bias_strength, mask)
        else:
            bg_test = _sample_background(cfg, bg_per_year)
        ...
        _visualise_points(pres_train, bounds, season, test_year, vis_dir, prefix="train")
        _visualise_points(bg_train, bounds, season, test_year, vis_dir, prefix="background")
...
# -----------------------------------------------------------------------------
def main() -> None:
    ap = argparse.ArgumentParser(description="Run monotemporal seasonal models")
