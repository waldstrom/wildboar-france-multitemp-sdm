"""Multitemporal positive control experiment that discards other predictors.

This script mirrors :mod:`scripts.multitemporal_classifier_control` but forces
the MaxEnt model to train **exclusively** on the synthetic ``positive_control``
layer.  All other predictors are ignored once the control layer has been
generated, providing a strict sanity check that the modelling workflow reacts
as expected when supplied with a perfectly predictive feature in isolation.
"""
...
# Controls whether the workflow keeps all predictors configured in the YAML
# file (``True``) or discards them to train exclusively on the synthetic
# ``positive_control`` layer (``False``).  The flag mirrors the behaviour of the
# multitemporal workflow when enabled.
INCLUDE_CONFIG_PREDICTORS: bool = True


# ---------------------------------------------------------------------------
def _safe_mahalanobis_thin(
    ...
    *,
    ...
    """Run Mahalanobis thinning when predictor data are available."""

    if env_matrix is None:
        log.warning(
            "Skipping Mahalanobis thinning for %s (no environmental matrix)", context
        )
        return gdf

    env_matrix = np.asarray(env_matrix)
    if env_matrix.ndim == 1:
        env_matrix = env_matrix.reshape(-1, 1)

    if env_matrix.size == 0 or env_matrix.shape[1] == 0:
        log.warning(
            "Skipping Mahalanobis thinning for %s (no predictor columns)", context
        )
        return gdf

    if env_matrix.shape[0] != len(gdf):
        log.warning(
            "Skipping Mahalanobis thinning for %s due to size mismatch (%d points vs %d predictors)",
            context,
            len(gdf),
            env_matrix.shape[0],
        )
    ...
    return mahalanobis_thin(gdf, target_n, env_matrix)
...
def _process_season(
    ...
    *,
    test_year: int,
    include_config_predictors: bool,
) -> None:
    """Run the multitemporal workflow for a single season.

    The logic mirrors :func:`scripts.multitemporal.process_season` but enforces
    the presence of the ``positive_control`` predictor.  When
    ``include_config_predictors`` is ``False`` the workflow discards every other
    variable before training the model, replicating the original positive-control
    behaviour.
    """
    ...
    raw_thin_cfg = cfg.get("presence_data", {}).get("thinning", {})
    if include_config_predictors:
        thin_cfg: dict[str, object] = (
            raw_thin_cfg if isinstance(raw_thin_cfg, dict) else {}
        )
    else:
        if isinstance(raw_thin_cfg, dict) and raw_thin_cfg:
            log.info(
                "Skipping configured thinning for zero-others positive control run",
            )
        thin_cfg = {}
    ...
    if rebuild or not stack_path.exists():
        ...
        if vars_fixed is None:
            vars_fixed = vars_used
    log.info("Initial predictor candidates: %s", ", ".join(vars_used))
    ...
    if thin_cfg.get("enable", False):
        ...
        if perc_map:
            for src, year_map in perc_map.items():
                mask_src = pres_train["source"].astype(str).to_numpy() == src
                ...
                if isinstance(year_map, dict):
                    for yr, pct in year_map.items():
                        mask_year = g_src["year"].to_numpy() == int(yr)
                        ...
                        if pct < 1.0:
                            ...
                            frames.append(
                                _safe_mahalanobis_thin(
                                    g_sub, tgt, env_sub, context=f"{src} {yr}"
                                )
                            )
                        else:
                            frames.append(g_sub)
            ...
            if pct < 1.0:
                ...
                pres_train = gpd.GeoDataFrame(
                    _safe_mahalanobis_thin(
                        pres_train, tgt, env, context="training presences"
                    ),
                    crs="EPSG:2154",
                )
    pres_train = deduplicate_cells(pres_train)
    ...
    positive_control_da(stack_path, pres_train, name=positive_control_name)
    if include_config_predictors:
        if positive_control_name not in vars_used:
            vars_used.append(positive_control_name)
        if vars_fixed is None:
            vars_fixed = list(vars_used)
        elif positive_control_name not in vars_fixed:
            vars_fixed.append(positive_control_name)
        log.info(
            "Retaining configured predictors and adding %s", positive_control_name
        )
    else:
        vars_used = [positive_control_name]
        vars_fixed = [positive_control_name]
        log.info(
            "Discarded non-control predictors; using only %s", positive_control_name
        )
    ...
    if positive_control_name in missing:
        missing.remove(positive_control_name)
    if missing:
        log.warning(
    ...
    write_positive_control_test_layer(test_stack_path, name=positive_control_name)
    ...
    if thin_cfg.get("enable", False):
        if "percentages" in thin_cfg:
            ...
            for src, year_map in season_map.items():
                mask_src = pres_test["source"].astype(str).to_numpy() == src
                g_src = pres_test.loc[mask_src].reset_index(drop=True)
                if len(g_src) == 0:
                    continue
                pct = year_map.get(test_year, 1.0) if isinstance(year_map, dict) else year_map
                if pct < 1.0 and len(g_src) > 3:
                    tgt = max(1, int(len(g_src) * pct))
                    env_src = env_test[mask_src]
                    frames.append(
                        _safe_mahalanobis_thin(
                            g_src, tgt, env_src, context=f"test {src}"
                        )
                    )
                else:
                    frames.append(g_src)
            ...
            if pct < 1.0 and len(pres_test) > 3:
                ...
                pres_test = gpd.GeoDataFrame(
                    _safe_mahalanobis_thin(
                        pres_test, tgt, env_test, context="test presences"
                    ),
                    crs="EPSG:2154",
                )
...
def _run_workflow(
    ...
    *,
    ...
    for season in tqdm(
        cfg_copy["multitemporal"]["seasons"], desc="Seasons", unit="season"
    ):
        log.info("Running season %s (positive control only)", season)
        _process_season(
            cfg_copy,
            season,
            rebuild,
            test_year=test_year,
            include_config_predictors=include_config_predictors,
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run multitemporal positive control (zero others) experiments for 2021"
    )
    ...
    parser.add_argument(
        "--zero-others",
        dest="include_config_predictors",
        action="store_false",
        help=(
            "Disable loading additional predictors from the configuration and "
            "train solely on the synthetic positive control layer."
        ),
    )
    parser.set_defaults(include_config_predictors=INCLUDE_CONFIG_PREDICTORS)
    ...
    _run_workflow(
        cfg,
        suffix="positive_control_zero_others",
        rebuild=args.rebuild,
        test_year=test_year,
        include_config_predictors=args.include_config_predictors,
    )
